...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.tree.heading('tel', text='Номер')
    ...
    def update_record(self, name, tel, email):
        self.db.conn.execute('''UPDATE db SET name= ?, tel= ?, email= ? WHERE ID = ?''', (name, tel, email,
...
class Update(Child):
    ...
    def init_edit(self):
        ...
        btn_edit.bind('<Button-1>', lambda event:
                      self.view.update_record(self.entry_name.get(), self.entry_email.get(), self.entry_tel.get()))
        btn_edit.bind('<Button-1>', lambda event:
                      self.destroy(), add='+')
        self.btn_ok.destroy()
